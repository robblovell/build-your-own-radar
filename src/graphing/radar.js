...
var Radar = function (size, radar) {
    ...
    var ringCalculator = new ringCalculator_1.default(radar.rings().length, center());
    var self = {};
    function center() {
    ...
    function plotQuadrant(rings, quadrant) {
        ...
        rings.forEach(function (ring, i) {
            var arc = d3.arc()
                ...
                .startAngle(toRadian(quadrant.startAngle - config_1.default.startLocation))
                .endAngle(toRadian(quadrant.startAngle + config_1.default.divisionAngle - 1 - config_1.default.startLocation));
            quadrantGroup.append('path')
                .attr('d', arc)
                .attr('class', 'ring-arc-' + ring.order())
                .attr('transform', 'translate(' + center() + ', ' + center() + ')'); // move to central location.
        });
        var angle = quadrant.startAngle + config_1.default.divisionAngle / 2; // - config.startLocation;
        // console.log(angle);
    ...
    function plotBlips(quadrantGroup, rings, quadrantWrapper) {
        ...
        rings.forEach(function (ring, i) {
            ...
            ringBlips.forEach(function (blip) {
                ...
                var coordinates = findBlipCoordinates(blip, minRadius, maxRadius, startAngle, allBlipCoordinatesInRing);
    ...
    function drawLegend(order, angle) {
        removeRadarLegend();
    ...
    function selectQuadrant(order, startAngle) {
        ...
        var scale = 1.5;
        var newSize = scale * size / 2;
        var angle = startAngle + config_1.default.divisionAngle / 2 - config_1.default.startLocation;
        var directionX = -Math.sin(toRadian(angle));
        var directionY = Math.cos(toRadian(angle));
        var adjustX = newSize / 2 * directionX;
        var adjustY = newSize / 2 * directionY;
        var adjustWindowX = -size / 5;
        var adjustWindowY = -size / 4;
        var translateX = adjustX + adjustWindowX;
        var translateY = adjustY + adjustWindowY;
        var scaleAll = 0;
        var translateXAll = 0;
        var translateYAll = 0;
        var moveRight = (window.innerWidth);
        var moveLeft = size / 2.4;
        var moveDown = window.innerHeight;
        var moveUp = 0;
        var blipScale = 3 / 4;
